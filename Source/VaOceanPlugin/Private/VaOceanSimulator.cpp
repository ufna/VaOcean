...
void AVaOceanSimulator::UpdateDisplacementMap(float WorldTime)
{
	...
	ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
		...
		FUpdateSpectrumCSPerFrame, PerFrameParams, UpdateSpectrumCSPerFrameParams,
		{
			...
			FUpdateSpectrumUniformBufferRef UniformBuffer = FUpdateSpectrumUniformBufferRef::CreateUniformBufferImmediate(Parameters, UniformBuffer_SingleFrame);
	...
	ENQUEUE_UNIQUE_RENDER_COMMAND_THREEPARAMETER(
		...
		FUpdateDisplacementPSPerFrame, PerFrameParams, UpdateDisplacementPSPerFrameParams,
		{
			...
			FIntPoint DisplacementMapResolution(TextureRenderTarget->GetSizeX(), TextureRenderTarget->GetSizeY());
			...
			GraphicsPSOInit.DepthStencilState = (TStaticDepthStencilState<false, CF_Always>::GetRHI());//Black vodo magic shit. Why the fucking hell are the two parethisis needed? The example I found worked fine with out them. I spent wayyy to long stuck on this line
			...
			RHICmdList.CopyToResolveTarget(
				TextureRenderTarget->GetRenderTargetTexture(),
	...
	//Test code for potentialy creating collision data from displacement renderTarget
	//TArray<FColor> SurfData;
	//FTextureRenderTargetResource* RenderTarget = DisplacementTexture->GameThread_GetRenderTargetResource();

	//RenderTarget->ReadPixels(SurfData);

	//FColor* colorPtr = SurfData.GetData();


	//FString colorStr = *colorPtr->ToHex();

	//UE_LOG(LogTemp, Warning, TEXT("%s"), *colorStr);
}
